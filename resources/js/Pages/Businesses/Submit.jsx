    ...
    <PublicLayout>
      ...
      <div className="max-w-2xl mx-auto p-6">
        ...
        <form onSubmit={submit} className="space-y-4" encType="multipart/form-data">
          <input className="w-full border border-gray-300 rounded-xl px-3 py-2" placeholder="Business name"
                 value={data.name} onChange={e => setData('name', e.target.value)} />
          {errors.name && <p className="text-red-600 text-sm">{errors.name}</p>}

          <input className="w-full border border-gray-300 rounded-xl px-3 py-2" placeholder="Owner / contact person"
                 value={data.owner_name} onChange={e => setData('owner_name', e.target.value)} />

          <input className="w-full border border-gray-300 rounded-xl px-3 py-2" placeholder="Contact number"
                 value={data.contact_number} onChange={e => setData('contact_number', e.target.value)} />

          <textarea className="w-full border border-gray-300 rounded-xl px-3 py-2" placeholder="Address"
